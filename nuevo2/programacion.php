...
$dattanaeur=$_SERVER['HTTP_HOST'];$dattanaeurcount = substr_count($dattanaeur, 'thwest');
if ($dattanaeurcount == '0'){session_destroy();header("location: index.php");};?>
...
<div class="bite"><a href="agregar-evento.php" class="baddb">.</a>PROYECTO<?php //echo $dias_cap;?></div>
...
// echo $fecha_hora1.'__    __'.$fecha_hora3;
 //echo 'frefre'.$fecha_hora2;
	...
	if ($_SESSION["tipo"] == 'vendedor'){
		$sql1 = "SELECT * FROM evento_act WHERE fecha>='".$fecha_hora1."' AND fecha<'".$fecha_hora2."' AND vendedor='".$_SESSION["vendedor"]."' AND checkedeve!='1' ORDER BY fecha ASC";
	}else{
		$sql1 = "SELECT * FROM evento_act WHERE fecha>='".$fecha_hora1."' AND fecha<'".$fecha_hora2."' AND checkedeve!='1' ORDER BY fecha ASC";
	};
...
<span class="titlecol tcolor eiix"<?php if(substr($row["fecha"],8,2) != $dia_hoy){echo ' style="margin-top:25px"';};?>><!--PROGRAMACIÓN --><?php echo replacedia_sem($dia_sem);?>
...
$fecha_hora2b = substr($row["fecha"],0,10).' 23:59:59';
		...
		if($_SESSION["tipo"] == 'admin'){		
			$sql1c = "SELECT * FROM evento_act WHERE fecha>='".$fecha_hora1b."' AND fecha<'".$fecha_hora2b."' AND checkedeve!='1' ORDER BY fecha ASC";		}
		else{		
			$sql1c = "SELECT * FROM evento_act WHERE fecha>='".$fecha_hora1b."' AND fecha<'".$fecha_hora2b."' AND vendedor='".$_SESSION["vendedor"]."' AND checkedeve!='1' ORDER BY fecha ASC";		
		};
		
		$result1c = mysql_query($sql1c, $conn1);
		$total_registros = mysql_num_rows($result1c);
?>
<span class="eventnum"><?php echo $total_registros;?></span> <span class="tamev">Actividad<?php if($total_registros > 1){echo 'es';};?></span>
</span> 



<?php if( $veces_text == 0){
		if($_SESSION["tipo"] == 'admin'){		$sql2 = "SELECT * FROM evento	";		}
		else{		$sql2 = "SELECT * FROM evento WHERE vendedor='".$_SESSION["vendedor"]."'	";		};
        $result2 = mysql_query($sql2, $conn1);
        $total_evento = mysql_num_rows($result2); 
	?>
<a href="cuenta.php" class="misproye">
<span class="tareum"><?= $total_evento;?></span> <span class="tamev"><?php if($_SESSION["tipo"] == 'admin'){?>Proyectos<?php }else{?>Tus proyectos<?php };?></span>
</a>
 <?php };?> 
